...
void printResult(Node* resultList){


    FILE *file;  
    file = fopen("Result.txt", "w"); //<-- open the file in the write mode.
    ...
    Node *currentNode1 = resultList;

    // if the result contain a negative sign print it :
    if(currentNode1->sign == '-'){
      fprintf(file, "%c", currentNode1->sign);
    }
    // print from left digit to right digit:
    while (currentNode1 != NULL){
      fprintf(file, "%d", currentNode1->data);
...
/*
the time complexity of this function is O(m*n*k) where:
m is the length of the first list (list1). 
n is the length of the second list (list2).
k is a constant time taken by insert, reverse, and Sum operations.
*/

Node* Multiplication(Node* list1, Node* list2) {
    // declare a list to hold and contain the result:
    ...
    // track each node in list2
    Node* currentNode2 = list2;
    int position = 0; //<-- position of the digit in list2

    while (currentNode2 != NULL) {
        // will create a new list to store the  result for the current digit in list2:
        Node* tempResult = NULL;
        int carry = 0; //<-- to hold the carry

        // multiply each digit in list1 with the current digit in list2 (by fixing digt in list 2 then ..)
        Node* currentNode1 = list1;
        while (currentNode1 != NULL) {
            int product = (currentNode1->data * currentNode2->data + carry) % 10; //<-- to store the result of list Node
            carry = (currentNode1->data * currentNode2->data + carry) / 10; //<-- to store the carry for the next loop
            insert(&tempResult, product); //<-- to insert product result into tempResult
            currentNode1 = currentNode1->next; //<-- move to the next node(int) in the list1.
        }

        // if there is a carry after multiplying all digits in list1 with the current digit in list2  just insert it:
        if (carry > 0) {
            insert(&tempResult, carry);
        }

        // add zeros to the temp result based on the position of the digit in list2
        for (int i = 0; i < position; i++) {
            insert(&tempResult, 0);
        }

        // add the tempresult to the final result:
        resultList = Sum(resultList, tempResult, resultList);

        currentNode2 = currentNode2->next; //<-- move to the next node(int) in the list1.
        position++; //<-- move the postion (to know if there any new zeros).
    }

    // reverse the final result to have the most significant digit at the head
    resultList = reverse(resultList);
    // to handle the sign for the multiplication
...
Node* LongDivision(Node* dividend, Node* divisor) {

    Node* quotient = NULL;  //<-- list will held the quotient of the division
    Node* remainder = NULL; //<-- list will held  remainder of the division

    // handle  division by zero or when the dividend is zero cases:
    if (divisor == NULL || divisor->data == 0) {
      // we can not deivide at zero:
        printf("Error: Division by zero\n");
        return NULL;
    }
    if (dividend == NULL || dividend->data == 0) {
        insert(&quotient, 0);
        return quotient;
    }
    ...
    Node* currentDigit = dividend; //<-- will keep traking the nodes (digit) of dividend list.
  ...
  while (!exitFlag){
    ...
    case 2:
      ...
      if (didRead == 0){
       ...
       // clear and delete the lists:
        ...
        resultList = Add_operations(list1, list2, resultList);
    ...
    case 5:
      ...
      if (didRead == 0){
        ...
        list2 = insert_line(number2, list2); //<-- to insert the 2nd selected int to the list & reverse it.
        display(list2);
        // do the devsion operation:
        printf("the result is :\n");
        resultList = LongDivision(list1, list2);
        display(resultList);
    ...
    case 6:
      ...
      if (resultList != NULL){
        ...
        printf("the result list is null");
      }
      break;
